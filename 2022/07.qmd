...
I build a helper function to locate the path of each line.

```{r}
cd <- function(path, dir = NA) {
    if(any(is.na(dir))) return(path)
    if(dir == "..") return(head(path, -1))
...
paths <- data %>% 
    ...
    mutate(path = c(accumulate(path, cd))) %>% 
    unnest(path)
...
used_space <- head(sizes, 1) %>% pull(size)

sizes %>% 
    filter(size >= (used_space + required_space - total_space)) %>% 
    arrange(size) %>% 
    head(1)
```

All the code for both parts:
